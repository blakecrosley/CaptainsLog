...
- The latest `Scripts/app_store_readiness_status.sh` run passed local readiness from clean `origin/main` and confirms no App Store Connect `.p8` private-key material is inside the repo. It still reports external App Store Connect credentials, app record, manual fields, upload/TestFlight, screenshot acceptance, legal review, and final real-account tap-through as open gates.
