...
- Manual App Store Connect fields still need to be entered and checked for missing-metadata warnings in App Store Connect, including the EU DSA trader-status declaration if EU availability remains enabled, the regulated-medical-device declaration if shown, and tax category if shown.
