...
- The latest `Scripts/app_store_readiness_status.sh` run passed preflight and the credential-guard self-test from clean source, but failed local readiness because the current IPA and export manifest are missing after the blocked clean export. IPA local-check is skipped until a current IPA exists. The script still confirms no App Store Connect `.p8` private-key material is inside the repo, reports the latest Vision compatible-launch smoke artifacts when present, reports the latest macOS launch smoke artifacts when present, and warns if either platform still has a local-only caveat. Candidate `AuthKey_*.p8` files are staged outside the repo under `~/.appstoreconnect/private_keys` with owner-only permissions, but the selected key ID and issuer UUID still need to be provided before export/upload credentials are usable. The script reports external App Store Connect credentials, app record, manual fields including Apple Vision Pro and Apple Silicon Mac availability, upload/TestFlight, screenshot acceptance, legal review, and final real-account tap-through as open gates.
...
- A compatible Apple Vision Pro path now has local evidence: the iOS target reports `SUPPORTS_XR_DESIGNED_FOR_IPHONE_IPAD=YES`, a Release build succeeded for a visionOS 26.5 simulator destination, `xcrun simctl install` installed the app, `xcrun simctl launch` returned process `87848` for `com.blakecrosley.captainslog`, and simulator screenshots were written at `3840x2160`. OCR on `/tmp/captainslog-vision-launch-late.png` read the first-run Captain's Log screen, Work Map preview, Sign in with GitHub, and Use Demo Data actions, but also read `Keychain returned status -34018.` `Scripts/smoke_vision_compatible_launch.sh /tmp/captainslog-vision-smoke` refreshes this build/install/launch/screenshot/OCR smoke path and warns on the same keychain text unless strict mode is enabled. The May 18, 2026 script run selected Vision simulator `37661BC9-7032-40F6-842F-1D2A647C2899`, built the no-sign Release compatible app, installed it, launched process `37042`, captured `/tmp/captainslog-vision-smoke/vision-compatible-launch.png` at `3840x2160`, and OCR found Captain's Log, Sign in with GitHub, Use Demo Data, and `Keychain returned status -34018`. This proves the compatible iPhone/iPad app renders its first-run UI on a Vision Pro simulator; it does not prove native visionOS, signed TestFlight behavior, App Store review metadata, GitHub auth/keychain success, or full Vision UX acceptance.
- A native macOS target exists with bundle ID `com.blakecrosley.captainslog.mac`, hardened runtime enabled, and deployment target macOS 26.0. A no-sign Release build for `CaptainsLog-macOS` succeeded on May 18, 2026 at `/tmp/captainslog-macos-release-build`; the built app reports version `1.0.0 (1)`, SDK `macosx26.5`, and category `public.app-category.developer-tools`. `Scripts/smoke_macos_launch.sh /tmp/captainslog-macos-smoke` refreshes this build/metadata/codesign/launch/quit smoke path. The May 18, 2026 script run rebuilt the no-sign Release app, recorded `com.blakecrosley.captainslog.mac`, version `1.0.0 (1)`, SDK `macosx26.5`, category `public.app-category.developer-tools`, launched process `47229`, and quit it cleanly. `codesign -dv` still reports `Signature=adhoc` and `TeamIdentifier=not set`, so Mac App Store signing/export, screenshots, metadata, TestFlight, and human QA remain open.
...
- Run `Scripts/upload_app_store_ipa.sh local-check <ipa>` before uploading. The local check requires the sibling `ExportManifest.txt`, rejects dirty-tree exports unless explicitly bypassed for legacy inspection, and fails if the submitted executable contains debug/test fixture strings, including screenshot hooks and UI-testing launch hooks. When App Store Connect API credentials are available, set `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` from App Store Connect, Transporter, or a manually authenticated altool session, then run `Scripts/upload_app_store_ipa.sh app-record` to verify the App Store Connect app record by bundle ID, followed by `Scripts/upload_app_store_ipa.sh validate <ipa>` and `Scripts/upload_app_store_ipa.sh upload <ipa>`. Xcode 26.5 `altool --list-providers` does not support API-key authentication. The script uses `xcrun altool` with API key authentication for app-record/validate/upload/status and keeps credentials in environment variables or Apple's supported private-key file locations outside the repo.
