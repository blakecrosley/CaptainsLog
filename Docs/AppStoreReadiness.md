...
- A local App Store Connect export succeeds with automatic signing. The current May 17, 2026 export from commit `33a03dc` produced `/tmp/captainslog-current-appstore-export/Export/Captain's Log.ipa`, uses `get-task-allow=false`, has `beta-reports-active=true`, includes bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `PrivacyInfo.xcprivacy`, and `ITSAppUsesNonExemptEncryption=false`. Upload to App Store Connect/TestFlight is still unverified.
