...
- A local App Store Connect export succeeds with automatic signing. The current May 17, 2026 export from commit `3b8f6bb` produced `/tmp/captainslog-current-appstore-export/Export/Captain's Log.ipa`, uses `get-task-allow=false`, includes bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `PrivacyInfo.xcprivacy`, and `ITSAppUsesNonExemptEncryption=false`. Upload to App Store Connect/TestFlight is still unverified.
...
- Export with App Store distribution signing before upload. The May 17, 2026 local export from commit `3b8f6bb` used `method=app-store-connect`, `destination=export`, automatic signing, and produced `/tmp/captainslog-current-appstore-export/Export/Captain's Log.ipa` with `get-task-allow=false`.
