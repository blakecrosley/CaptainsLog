...
- A native macOS target exists with bundle ID `com.blakecrosley.captainslog.mac`, hardened runtime enabled, and deployment target macOS 26.0. A no-sign Release build for `CaptainsLog-macOS` succeeded on May 18, 2026 at `/tmp/captainslog-macos-release-build`. This is compile evidence only; Mac App Store signing/export, screenshots, metadata, TestFlight, and human QA remain open.
