...
- A recent Debug build succeeds for the connected iPhone 17 Pro Max running iOS 26.4.2, installs through `xcrun devicectl`, and launches with bundle ID `com.blakecrosley.captainslog`. The latest post-polish physical-device UI-test rerun was blocked by the locked phone, so final Work Map polish verification is simulator, screenshot, export, and preflight based.
...
- Connected-device UI tests run on Blake's iPhone 17 Pro Max and previously passed 2 tests with 0 failures. The latest current-head simulator refresh on iOS 26.5 passed 69 unit tests and 2 `CaptainsLogUITests` UI tests with 0 failures, covering first-run Sign in with GitHub and Use Demo Data actions, fixture dashboard launch, Settings, Privacy & Data, and selected-day journal detail.
