...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Scripts/app_store_readiness_status.sh`, `Scripts/smoke_vision_compatible_launch.sh`, `Scripts/smoke_macos_launch.sh`, `Scripts/smoke_watchos_launch.sh`, `Scripts/smoke_tvos_launch.sh`, `Scripts/capture_watchos_app_store_screenshots.sh`, `Scripts/capture_tvos_app_store_screenshots.sh`, generic no-sign watchOS/tvOS `xcodebuild archive`, `Scripts/check_app_store_connect_record.py --bundle-id com.blakecrosley.captainslog.watchkitapp --entitlements CaptainsLogCompanion/CaptainsLogCompanion.entitlements --skip-app-record --require capabilities`, `Scripts/ensure_platform_bundle_ids.py`, `Scripts/export_macos_app_store_pkg.sh`, `Docs/AppStoreReadiness.md`, `../Return/Return.xcodeproj`, `../Banana List/Banana List.xcodeproj` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance. A native macOS target and a tvOS target exist locally, but Apple current guidance for a single app record/universal purchase says added macOS, tvOS, and visionOS platform versions share the iOS app bundle ID. The local precedents match that: ReturnTV uses `com.941apps.Return`, and Get Bananas uses `com.941apps.Banana-List` for its main iOS/Mac app. Captain's Log currently uses separate `.mac` and `.tv` bundle IDs, so those account mutations are intentionally guarded until the distribution model is chosen. The Watch companion is different: Captain's Log uses `com.blakecrosley.captainslog.watchkitapp`, matching Return/Get Bananas `.watchkitapp` precedent, but the remote Watch bundle ID/capability proof is still missing. `Scripts/ensure_platform_bundle_ids.py` dry-runs all configured targets, requires `--target watchos --apply --confirm-team M4WTLM6RAQ` for the straightforward Watch companion mutation, and requires the extra `--confirm-separate-platform-records` flag before applying separate Mac/TV bundle IDs. Captain's Log still needs the iOS app record, correct platform bundle-ID model, required capabilities, signed builds/exports, metadata, TestFlight, provisioning validation, human screenshot acceptance, and platform QA before Mac, Watch, or TV can be called ready. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/TV distribution model unresolved; Watch remote bundle ID missing |
...
| Native Mac, Watch, and TV bundle IDs | For Watch, run `Scripts/ensure_platform_bundle_ids.py --target watchos --apply --confirm-team M4WTLM6RAQ` only after explicit Apple account mutation approval. For Mac and TV, first choose whether Captain's Log should follow Apple's single app record/universal-purchase model using the iOS bundle ID or intentionally ship separate app records using the current `.mac` and `.tv` target bundle IDs. | Watch evidence closes when the approved apply command exits successfully with post-apply verification, or `Scripts/check_app_store_connect_record.py --bundle-id com.blakecrosley.captainslog.watchkitapp --entitlements CaptainsLogCompanion/CaptainsLogCompanion.entitlements --skip-app-record --require capabilities` passes. Mac/TV evidence closes only after the distribution model is chosen and matching Xcode/account checks pass. |
