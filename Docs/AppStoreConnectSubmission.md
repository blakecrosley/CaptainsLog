...
- Apple says iPhone and iPad apps are available on Apple Vision Pro by default unless availability is edited in App Store Connect. Captain's Log now has local compatible-app evidence on Vision Pro: a Release build, simulator install, simulator launch, and OCR reading the first-run UI in `/tmp/captainslog-vision-launch-late.png`. That same OCR also read `Keychain returned status -34018`, so the first-submission recommendation is to leave Vision Pro availability enabled as the compatible iPhone/iPad app only after final smoke-test acceptance, without claiming a native visionOS app.
...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Docs/AppStoreReadiness.md` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance; a native macOS target exists but is not App Store ready; no Apple Watch or Apple TV app target or scheme exists. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/watch/tv not ready |
