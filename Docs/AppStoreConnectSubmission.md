...
- `Scripts/capture_app_store_screenshots.sh /tmp/captainslog-key-state-audit`, `Scripts/package_app_store_screenshots.sh /tmp/captainslog-key-state-audit /tmp/captainslog-key-state-packaged`, and `Scripts/make_app_store_screenshot_contact_sheet.sh /tmp/captainslog-key-state-packaged /tmp/captainslog-appstore-review` regenerated the current 12-screenshot package after cleaning the demo journal copy. A Vision OCR scan over the packaged screenshots found no `fixture`, `UI Fixture`, debug, simulator, sync-progress, error, personal-account, or token-like text.
