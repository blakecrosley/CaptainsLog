...
- `CAPTAINS_LOG_REQUIRE_CLEAN_EXPORT=1 Scripts/export_app_store_ipa.sh /tmp/captainslog-current-appstore-export` now checks Xcode 26+, an iOS 26+ SDK, and distribution signing for team `M4WTLM6RAQ` before archiving. It can also pass `APP_STORE_CONNECT_API_KEY`, `APP_STORE_CONNECT_API_ISSUER`, and `APP_STORE_CONNECT_P8_FILE` to `xcodebuild` for App Store Connect-authenticated provisioning updates when those env vars are set. A prior selected 941-app API-key pattern passed shape/file checks and reached Apple during export, but the May 19, 2026 signing-status check in this shell reports those API-key env vars are not set. A May 18, 2026 clean export attempt with selected API-key inputs archived the generic iOS app with Apple Development signing, then `xcodebuild -exportArchive` failed with `Cloud signing permission error` and `No signing certificate "iOS Distribution" found`; `/tmp/captainslog-current-appstore-export` remained empty. The current iOS export is blocked until either the API-key environment is selected and has cloud-managed distribution certificate access, or local Xcode has an Apple Distribution/iOS Distribution signing identity with private key.
...
- `Scripts/upload_app_store_ipa.sh app-record` and `Scripts/check_app_store_connect_record.py` do not require an IPA or an altool provider public ID. A May 18, 2026 REST API check with the selected App Store Connect API-key inputs found the Developer Portal bundle ID `com.blakecrosley.captainslog` for team `M4WTLM6RAQ`, but found no App Store Connect app record for that bundle ID. The app record must be created or made visible to this API key before the first upload path can be considered ready. `Scripts/upload_app_store_ipa.sh app-record-altool` remains available for the older `altool --list-apps` path when `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` is known.
...
| --- | --- | --- |
| App Store Connect app record | Create or confirm the iOS app record with bundle ID `com.blakecrosley.captainslog`, SKU `captainslog-ios`, primary language English (U.S.), and team `M4WTLM6RAQ`. Current REST evidence finds the Developer Portal bundle ID but no App Store Connect app record. | `Scripts/check_app_store_connect_record.py` lists the app by bundle ID, and the Apple ID is captured as `APP_STORE_CONNECT_APPLE_ID` for status checks. |
