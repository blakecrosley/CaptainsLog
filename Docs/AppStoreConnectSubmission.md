...
| Metadata ready to paste | `Docs/AppStoreMetadata.md` | Name, subtitle, description, keywords, review notes, URLs, screenshot order, privacy draft, and manual App Store Connect choices for pricing, availability, EU DSA trader status, regulated medical device status, tax category, release option, age rating, content rights, Made for Kids, App Review contact, demo account, and accessibility-label handling | Locally ready, legal/privacy review open |
