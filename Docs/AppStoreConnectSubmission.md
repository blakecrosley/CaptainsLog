...
- `Scripts/app_store_readiness_status.sh` failed on May 19, 2026 because the current IPA and export manifest are missing after the blocked clean export; IPA local-check is skipped until the IPA exists. It passed preflight, passed the credential-guard self-test, verifies that no App Store Connect `.p8` private-key material is inside the repository, verifies no OpenAI/Anthropic/GitHub-token-shaped source literals are present in app, test, doc, or script files, reports 4 candidate `.p8` files staged outside the repo, and reports the remaining external App Store Connect gates. The missing current IPA is now specifically blocked by cloud-managed distribution certificate access or local Apple Distribution/iOS Distribution identity installation, not by lack of a known API-key file.
