...
| Distribution certificates and App Store profiles | Create or make visible the required distribution certificate path, regenerate/download active App Store provisioning profiles for `com.blakecrosley.captainslog`, and create the Watch companion bundle ID only after explicit Apple account mutation approval. Current remote evidence reports 0 visible usable iOS distribution certificates, 0 visible usable Mac App Store application certificates, 0 visible usable Mac installer certificates, an `INVALID` iOS `IOS_APP_STORE` profile, missing active iOS/tvOS/Mac App Store profiles for the shared bundle ID, and a missing Watch bundle ID. | `Scripts/check_remote_signing_assets.py --require` reports the required certificate groups and active profile types for the selected targets, and `Scripts/export_app_store_ipa.sh` produces a current signed IPA plus `ExportManifest.txt`. For Mac, `Scripts/export_macos_app_store_pkg.sh` produces a signed package plus `MacExportManifest.txt`. |
