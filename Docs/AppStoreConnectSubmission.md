...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Scripts/smoke_vision_compatible_launch.sh`, `Docs/AppStoreReadiness.md` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance; a native macOS target exists but is not App Store ready; no Apple Watch or Apple TV app target or scheme exists. The Vision smoke script rebuilds, installs, launches, screenshots, and OCRs the compatible app on a Vision simulator. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/watch/tv not ready |
