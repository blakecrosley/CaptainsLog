...
- Current local Kit941 HEAD: `cdc93e2 Use theme accent in Playground catalog` (clean, ahead of `origin/main` by 4 commits). The unpushed set includes linked library-source commit `6f66986 Route default accents through Kit941 theme`, so final export needs Kit941 pushed or explicitly accepted as unpushed local source input.
...
- The export manifest records the local `Kit941` package commit and dirty state because the app links `../941Kit` directly. Current readiness shows Kit941 clean but locally ahead of `origin/main` by 4 commits, including linked library-source commit `6f66986 Route default accents through Kit941 theme` and playground-only commit `cdc93e2 Use theme accent in Playground catalog`. A current no-sign generic iOS Release build succeeded at `/tmp/captainslog-current-ios-release-kit941-cdc93e2`, but final export still needs Kit941 pushed or the unpushed local source state explicitly accepted.
...
| Linked package source traceability | Push or intentionally accept any clean local CaptainsLog/Kit941 commits before the final release build. | `Scripts/app_store_readiness_status.sh` shows whether CaptainsLog and Kit941 are synced with upstream. CaptainsLog is currently clean and synced with `origin/main`. Kit941 is currently clean but locally ahead of `origin/main` by 4 commits at `cdc93e2`, including linked library-source commit `6f66986`. The export manifest must record the exact local commit, and the release owner must either push Kit941 or explicitly accept the unpushed source state before final export. |
