...
| Final Add for Review gate | After the build is processed, follow `Docs/AppStoreConnectRunbook.md` step 8: select build `1.0.0 (1)`, confirm privacy, pricing, country/region availability, Apple Vision Pro availability, age rating, content rights, release option, license, regional/compliance prompts, screenshots, review contact, review notes, demo credentials if used, legal/privacy approval, and screenshot approval. | App Store Connect shows no missing-metadata warnings and allows the version to be added for review. |
