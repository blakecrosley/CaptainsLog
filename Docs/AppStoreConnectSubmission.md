...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Scripts/app_store_readiness_status.sh`, `Scripts/smoke_vision_compatible_launch.sh`, `Scripts/smoke_macos_launch.sh`, `Scripts/check_app_store_connect_record.py --bundle-id com.blakecrosley.captainslog.mac --require bundle-id`, `Scripts/export_macos_app_store_pkg.sh`, `Docs/AppStoreReadiness.md`, `../Return/Return.xcodeproj`, `../Banana List/Banana List.xcodeproj` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance; a native macOS target exists with automatic signing build settings for team `M4WTLM6RAQ` but is not App Store ready and its native Mac Developer Portal bundle ID is missing or not visible to the current API key; no Apple Watch or Apple TV app target or scheme exists. Existing 941 apps are useful precedents, not substitute evidence: readiness now checks that Return has `ReturnWatch Watch App` and `ReturnTV` targets/schemes, and that Get Bananas has a watch app target/scheme through the local `Banana List` Xcode project. Captain's Log still needs its own watchOS/tvOS targets, bundle IDs, signed builds, screenshots, metadata, TestFlight, and human QA before those platforms can be called ready. The linked `Kit941` package includes reusable tvOS focus styling, but Captain's Log still has no tvOS app target, scheme, signed build, screenshots, or review path. The Vision smoke script rebuilds, installs, launches, screenshots, and OCRs the compatible app on a Vision simulator. The macOS smoke script rebuilds the native Mac target, records metadata/codesign, launches, records a process, and quits it. The Mac export script stages a native Mac App Store archive/package and manifest after signing/auth is available. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/watch/tv not ready |
...
Return and Get Bananas use Fastlane aliases for the same App Store Connect inputs: `ASC_KEY_ID`, `ASC_ISSUER_ID`, and `ASC_KEY_PATH`. If reusing that setup, map them to Captain's Log's names only after confirming the key file path is outside every git working tree:
