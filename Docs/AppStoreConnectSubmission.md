...
- The export manifest records the local `Kit941` package commit and dirty state because the app links `../941Kit` directly. Current readiness shows Kit941 clean but `## main...origin/main [ahead 1]`; readiness classifies that drift as outside linked package source, and future linked package-source drift is still reported before export/submission work.
...
| Linked package source traceability | Push or intentionally accept any clean local CaptainsLog/Kit941 package-source commits before the final release build. | `Scripts/app_store_readiness_status.sh` shows whether CaptainsLog and Kit941 are synced with upstream. CaptainsLog is currently synced with `origin/main`; Kit941 is clean but `## main...origin/main [ahead 1]`, which readiness classifies as drift outside linked package source. If compiled package source is ever unsynced, the export manifest must record the exact local commit and the release owner must explicitly accept that state. |
