...
- `Scripts/upload_app_store_ipa.sh app-record` does not require an IPA and can be run as soon as App Store Connect API credentials and `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` are set. Xcode 26.5 `altool --list-apps` requires `--provider-public-id`, but `altool --list-providers` does not support API-key authentication, so obtain the provider public ID from App Store Connect, Transporter, or a manually authenticated altool session. The helper fails unless the filtered app-record output contains bundle ID `com.blakecrosley.captainslog`.
...
| Binary export ready | `Scripts/export_app_store_ipa.sh` | Export is blocked until one export-signing path is configured: local App Store account/distribution signing or App Store Connect API-key env vars for `xcodebuild` provisioning updates. The export script checks Xcode 26+, iOS 26+ SDK, signing/auth, clean CaptainsLog source, and clean linked Kit941 package inputs before archiving, then stages archive, IPA, and export manifest together and only replaces the current IPA folder after export validation succeeds. Dirty Kit941 files outside `Package.swift`, `Package.resolved`, and `Sources/Kit941` are recorded in the manifest instead of blocking export. Bypassing the precheck proved archive can succeed, but App Store export still fails without account/auth access. | Blocked on signing/auth credentials |
