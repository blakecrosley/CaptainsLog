...
- `CAPTAINS_LOG_REQUIRE_CLEAN_EXPORT=1 Scripts/export_app_store_ipa.sh /tmp/captainslog-current-appstore-export` now checks Xcode 26+, an iOS 26+ SDK, and distribution signing for team `M4WTLM6RAQ` before archiving. It can also pass `APP_STORE_CONNECT_API_KEY`, `APP_STORE_CONNECT_API_ISSUER`, and `APP_STORE_CONNECT_P8_FILE` to `xcodebuild` for App Store Connect-authenticated provisioning updates when those env vars are set. The selected local 941-app API-key pattern passes shape/file checks and reaches Apple during export, but the current iOS export is still blocked because local Xcode cannot find an Apple Distribution/iOS Distribution signing identity and the App Store Connect account does not have cloud-managed distribution certificate access. A May 18, 2026 clean export attempt with selected API-key inputs archived the generic iOS app with Apple Development signing, then `xcodebuild -exportArchive` failed with `Cloud signing permission error` and `No signing certificate "iOS Distribution" found`; `/tmp/captainslog-current-appstore-export` remained empty.
