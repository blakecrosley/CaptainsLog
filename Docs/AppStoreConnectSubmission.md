...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Scripts/app_store_readiness_status.sh`, `Scripts/smoke_vision_compatible_launch.sh`, `Scripts/smoke_macos_launch.sh`, `Scripts/smoke_watchos_launch.sh`, `Scripts/smoke_tvos_launch.sh`, `Scripts/capture_watchos_app_store_screenshots.sh`, `Scripts/capture_tvos_app_store_screenshots.sh`, generic no-sign watchOS/tvOS `xcodebuild archive`, `Scripts/check_app_store_connect_record.py --bundle-id com.blakecrosley.captainslog.watchkitapp --entitlements CaptainsLogCompanion/CaptainsLogCompanion.entitlements --skip-app-record --require capabilities`, `Scripts/ensure_platform_bundle_ids.py`, `Scripts/export_macos_app_store_pkg.sh`, `Docs/AppStoreReadiness.md`, `../Return/Return.xcodeproj`, `../Banana List/Banana List.xcodeproj` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance. Native macOS and tvOS targets now share the iOS bundle ID `com.blakecrosley.captainslog`, matching Apple's single app record/universal-purchase guidance and the local Return/Get Bananas precedent. The Watch companion is different: Captain's Log uses `com.blakecrosley.captainslog.watchkitapp`, matching Return/Get Bananas `.watchkitapp` precedent, but the remote Watch bundle ID/capability proof is still missing. `Scripts/ensure_platform_bundle_ids.py` dry-runs Mac/TV as already using the iOS bundle ID and requires `--target watchos --apply --confirm-team M4WTLM6RAQ` for the straightforward Watch companion mutation. Captain's Log still needs the iOS app record, required capabilities, signed builds/exports, metadata, TestFlight, provisioning validation, human screenshot acceptance, and platform QA before Mac, Watch, or TV can be called ready. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/TV bundle model aligned; Watch remote bundle ID missing |
