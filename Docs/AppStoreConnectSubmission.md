...
- The export manifest records the local `Kit941` package commit and dirty state because the app links `../941Kit` directly. Current readiness shows Kit941 clean but `## main...origin/main [ahead 1]`; `git -C ../941Kit diff --name-only origin/main..HEAD` shows the ahead commit is limited to `Playground/PlaygroundApp.swift`, outside the linked `Kit941` library product source. Future linked library-source drift is still reported before export/submission work.
...
| Linked package source traceability | Push or intentionally accept any clean local CaptainsLog/Kit941 library-source commits before the final release build. | `Scripts/app_store_readiness_status.sh` shows whether CaptainsLog and Kit941 are synced with upstream. CaptainsLog is currently synced with `origin/main`; Kit941 is clean but `## main...origin/main [ahead 1]` at `0a19f967dc59`, and the ahead diff is limited to `Playground/PlaygroundApp.swift`, outside the linked `Kit941` library product source. If compiled library source is ever unsynced, the export manifest must record the exact local commit and the release owner must explicitly accept that state. |
