...
- `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-uitesting-gate-tests -only-testing:CaptainsLogTests` passed 69 unit tests with 0 failures on the iOS 26.5 simulator from current `main`.
- `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-period-menu-uitests -only-testing:CaptainsLogUITests` passed 2 UI tests with 0 failures on the iOS 26.5 simulator from current `main` after the compact dashboard period-control polish. This covers first-run primary actions, fixture dashboard launch, settings navigation, Privacy & Data, and selected-day journal detail.
- `xcodebuild build -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -configuration Release -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-uitesting-gate-release` succeeded on the iOS 26.5 simulator. The built Release executable did not contain the debug/test fixture strings rejected by `Scripts/upload_app_store_ipa.sh local-check`, including screenshot hooks, debug OpenAI key seeding hooks, UI-testing hooks, and the screenshot demo key. Earlier bundle metadata also reported `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `ITSAppUsesNonExemptEncryption=false`, and `PrivacyInfo.xcprivacy` present. A signed IPA local-check is still required after App Store export succeeds.
