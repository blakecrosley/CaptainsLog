...
- `Scripts/app_store_readiness_status.sh` failed on May 18, 2026 because the current IPA and export manifest are missing after the blocked clean export; IPA local-check is skipped until the IPA exists. It passed preflight, passed the credential-guard self-test, verifies that no App Store Connect `.p8` private-key material is inside the repository, and reports the remaining external App Store Connect gates.
