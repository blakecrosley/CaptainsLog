...
Do not treat existing App Store Connect records for Return, Banana List, or other 941 apps as evidence that this step is closed for Captain's Log. Those apps prove the account can carry shipped records, but Captain's Log still needs its own `com.blakecrosley.captainslog` app record visible to the selected API key.
