...
- Apple says users on Macs with Apple silicon can access iPhone and iPad apps through the Mac App Store unless availability is edited. Captain's Log has a separate native macOS target and an iOS-on-Mac destination. The native target now has automatic signing build settings for team `M4WTLM6RAQ` plus local smoke and screenshot evidence, but it still does not have a signed Mac App Store export, TestFlight pass, or human QA acceptance, so the first-submission recommendation is to opt out unless a Mac/TestFlight pass is completed. `Scripts/smoke_macos_launch.sh /tmp/captainslog-macos-smoke` refreshes the local native Mac build/launch proof, but it is not Mac App Store readiness by itself.
...
| Platform availability | `project.yml`, `xcodebuild -list`, `xcodebuild -showBuildSettings`, `Scripts/smoke_vision_compatible_launch.sh`, `Scripts/smoke_macos_launch.sh`, `Docs/AppStoreReadiness.md` | iPhone and iPad are enabled through the iOS target family `1,2`; Apple Vision Pro should use the compatible iPhone/iPad app path after final smoke-test acceptance; a native macOS target exists with automatic signing build settings for team `M4WTLM6RAQ` but is not App Store ready; no Apple Watch or Apple TV app target or scheme exists. The Vision smoke script rebuilds, installs, launches, screenshots, and OCRs the compatible app on a Vision simulator. The macOS smoke script rebuilds the native Mac target, records metadata/codesign, launches, records a process, and quits it. | iPhone/iPad locally ready; Vision compatible path locally launched; Mac/watch/tv not ready |
