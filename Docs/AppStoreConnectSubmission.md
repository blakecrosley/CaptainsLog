...
- Apple says iPhone and iPad apps are available on Apple Vision Pro by default unless availability is edited in App Store Connect. Captain's Log now has local compatible-app evidence on Vision Pro: a Release build, simulator install, simulator launch, and OCR reading the first-run UI in `/tmp/captainslog-vision-launch-late.png`. `Scripts/smoke_vision_compatible_launch.sh /tmp/captainslog-vision-smoke` refreshes that same path. The prior OCR also read `Keychain returned status -34018`, so the first-submission recommendation is to leave Vision Pro availability enabled as the compatible iPhone/iPad app only after final smoke-test acceptance, without claiming a native visionOS app.
