...
| Binary export ready | `Scripts/export_app_store_ipa.sh` | Export is blocked by missing App Store account/distribution signing; the export script checks Xcode 26+, iOS 26+ SDK, and signing before archiving, then stages archive, IPA, and export manifest together and only replaces the current IPA folder after export validation succeeds. Bypassing the precheck proved archive can succeed, but App Store export still fails with `No Accounts` and missing `iOS Distribution`. | Blocked on signing credentials/certificate |
