...
- `Scripts/upload_app_store_ipa.sh app-record` does not require an IPA and can be run as soon as App Store Connect API credentials and `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` are set. The API-key inputs are now locally identifiable from the existing 941-app pattern, but Xcode 26.5 `altool --list-apps` still requires `--provider-public-id`; `altool --list-providers` does not support API-key authentication, so obtain the provider public ID from App Store Connect, Transporter, or a manually authenticated altool session. The helper fails unless the filtered app-record output contains bundle ID `com.blakecrosley.captainslog`.
- `Scripts/upload_app_store_ipa.sh validate "/tmp/captainslog-current-appstore-export/Export/Captain's Log.ipa"` cannot be rerun until the current IPA exists. After export succeeds, validation still needs the selected App Store Connect API-key environment and provider-public-ID context for the app-record/upload flow.
...
After App Store Connect API credentials are set, export `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` for the provider that owns `com.blakecrosley.captainslog` before `app-record`. Xcode 26.5 `altool --list-providers` does not support API-key authentication, so obtain the provider public ID from App Store Connect, Transporter, or a manually authenticated altool session.
...
| --- | --- | --- |
| App Store Connect app record | Create or confirm the iOS app record with bundle ID `com.blakecrosley.captainslog`, SKU `captainslog-ios`, primary language English (U.S.), and team `M4WTLM6RAQ`. Set `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` before checking the record; Xcode 26.5 `altool --list-providers` does not support API-key authentication, so obtain the provider public ID from App Store Connect, Transporter, or a manually authenticated altool session. | `Scripts/upload_app_store_ipa.sh app-record` lists the app by bundle ID, and the Apple ID is captured as `APP_STORE_CONNECT_APPLE_ID` for status checks. |
