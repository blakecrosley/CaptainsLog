...
| Binary export ready | `Scripts/export_app_store_ipa.sh` | Export is blocked until one export-signing path is complete: local App Store account/distribution signing with private key, or App Store Connect API-key provisioning with cloud-managed distribution certificate access. The export script checks Xcode 26+, iOS 26+ SDK, signing/auth, clean CaptainsLog source, and clean linked Kit941 package inputs before archiving, then stages archive, IPA, and export manifest together and only replaces the current IPA folder after export validation succeeds. Dirty Kit941 files outside `Package.swift`, `Package.resolved`, and `Sources/Kit941` are recorded in the manifest instead of blocking export. The current repo-local gitignored env selects API-key inputs for default checks, but export still fails until cloud distribution-certificate access or matching local distribution identities and App Store provisioning profiles are available. | Blocked on signing/certificate access |
