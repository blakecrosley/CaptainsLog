...
- `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-current-head-tests -only-testing:CaptainsLogTests` passed 69 unit tests with 0 failures on the iOS 26.5 simulator from current `main`.
- `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-current-head-tests -only-testing:CaptainsLogUITests` passed 2 UI tests with 0 failures on the iOS 26.5 simulator from current `main`. This covers first-run primary actions, fixture dashboard launch, settings navigation, Privacy & Data, and selected-day journal detail.
