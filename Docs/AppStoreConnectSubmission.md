...
- `xcodebuild archive -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -configuration Release -destination 'generic/platform=iOS' -archivePath /tmp/captainslog-current-nosign-archive.xcarchive -derivedDataPath /tmp/captainslog-current-nosign-archive-derived CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY=''` succeeded on May 18, 2026 from current source with the linked local `Kit941` package. The archived app metadata reported `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `ITSAppUsesNonExemptEncryption=false`, `PrivacyInfo.xcprivacy` present, and `Assets.car` present. This is unsigned compile/package evidence only; it does not produce the App Store IPA or close the distribution-signing gate.
- A connected-phone `CaptainsLogUITests` run previously passed 2 UI tests with 0 failures on Blake's iPhone 17 Pro Max. The May 18, 2026 current-head connected-phone retry used `/tmp/captainslog-current-device-uitests-fresh`, built and code signed the app plus test runner with Apple Development, then stopped at `Unlock Blake's iPhone 17 Pro Max (current) to Continue`; the hanging run was interrupted after the destination remained locked. The log is `/tmp/captainslog-current-device-uitests-fresh.log`. Final Work Map polish is simulator, screenshot, export, and preflight verified rather than fresh physical-device UI-test verified.
...
| Current source archives for device | `/tmp/captainslog-current-nosign-archive.xcarchive` | The May 18, 2026 no-sign generic iOS Release archive succeeded with the linked local `Kit941` package and contains the expected bundle ID, version/build, export-compliance flag, privacy manifest, and asset catalog. | Compile/package proof only; signed IPA still blocked |
