...
- `CAPTAINS_LOG_REQUIRE_CLEAN_EXPORT=1 Scripts/export_app_store_ipa.sh /tmp/captainslog-current-appstore-export` now checks Xcode 26+, an iOS 26+ SDK, and distribution signing for team `M4WTLM6RAQ` before archiving. It can also pass `APP_STORE_CONNECT_API_KEY`, `APP_STORE_CONNECT_API_ISSUER`, and `APP_STORE_CONNECT_P8_FILE` to `xcodebuild` for App Store Connect-authenticated provisioning updates when those env vars are set. Current export is still blocked because local Xcode cannot find an Apple Distribution/iOS Distribution signing identity and the App Store Connect API env vars are not set. A prior `CAPTAINS_LOG_SKIP_DISTRIBUTION_SIGNING_PRECHECK=1` attempt archived the generic iOS app with Apple Development signing, then `xcodebuild -exportArchive` failed with `No Accounts` and `No signing certificate "iOS Distribution" found`.
...
- `Scripts/app_store_readiness_status.sh` failed on May 18, 2026 because the current IPA and export manifest are missing after the blocked clean export; IPA local-check is skipped until the IPA exists. It passed preflight, passed the credential-guard self-test, verifies that no App Store Connect `.p8` private-key material is inside the repository, verifies no OpenAI/Anthropic/GitHub-token-shaped source literals are present in app, test, doc, or script files, and reports the remaining external App Store Connect gates.
