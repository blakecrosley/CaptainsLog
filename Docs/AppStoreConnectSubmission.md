...
- Apple says users on Macs with Apple silicon can access iPhone and iPad apps through the Mac App Store unless availability is edited. Captain's Log has a separate native macOS target and an iOS-on-Mac destination. The native target now has automatic signing build settings for team `M4WTLM6RAQ` plus local smoke, screenshot, and export-helper evidence, but it still does not have a signed Mac App Store export, TestFlight pass, or human QA acceptance, so the first-submission recommendation is to opt out unless a Mac/TestFlight pass is completed. `Scripts/smoke_macos_launch.sh /tmp/captainslog-macos-smoke` refreshes the local native Mac build/launch proof. A May 18, 2026 precheck-bypassed `Scripts/export_macos_app_store_pkg.sh /tmp/captainslog-current-macos-appstore-export` run reached `xcodebuild`, archived the native Mac app with Apple Development signing, then failed export with `No Accounts`, no `Mac Installer Distribution` certificate, and no profiles for `com.blakecrosley.captainslog.mac`; the package path can close only after Mac App Store signing/auth is available. Neither local Mac smoke nor an Apple Development-signed archive is Mac App Store readiness by itself.
