...
- `xcodebuild build -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -configuration Release -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-release-string-check` succeeded on the iOS 26.5 simulator. The built Release executable did not contain the debug screenshot/auth fixture strings rejected by `Scripts/upload_app_store_ipa.sh local-check`, and the bundle reported `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `ITSAppUsesNonExemptEncryption=false`, and `PrivacyInfo.xcprivacy` present. A signed IPA local-check is still required after App Store export succeeds.
