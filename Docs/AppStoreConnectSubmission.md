...
- `Scripts/upload_app_store_ipa.sh app-record` and `Scripts/check_app_store_connect_record.py` do not require an IPA or an altool provider public ID. A May 19, 2026 REST API check with the selected App Store Connect API-key inputs found the Developer Portal bundle ID `com.blakecrosley.captainslog` for team `M4WTLM6RAQ` and confirmed the required `ICLOUD` bundle capability is enabled, but found no App Store Connect app record for that bundle ID. The app record must be created or made visible to this API key before the first upload path can be considered ready. `Scripts/upload_app_store_ipa.sh app-record-altool` remains available for the older `altool --list-apps` path when `APP_STORE_CONNECT_PROVIDER_PUBLIC_ID` is known.
