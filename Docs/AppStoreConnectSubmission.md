...
- Current local Kit941 HEAD: `f06a0750122f` (clean, ahead of `origin/main` by `Playground/PlaygroundApp.swift` only). Current linked Kit941 library source still matches `origin/main` at `cbac28644070093dee23e9026df17a011fafff59`.
...
- The export manifest records the local `Kit941` package commit and dirty state because the app links `../941Kit` directly. Current readiness shows Kit941 clean but locally ahead of `origin/main` by a Playground-only commit, so linked package source is not currently ahead of upstream. The latest linked-source commit still refines `TVFocusButtonStyle` focus contrast at `cbac28644070093dee23e9026df17a011fafff59` and was followed by a no-sign generic iOS Release build at `/tmp/captainslog-current-ios-release-cbac`. Future linked library-source drift is still reported before export/submission work.
...
| Linked package source traceability | Push or intentionally accept any clean local CaptainsLog/Kit941 library-source commits before the final release build. | `Scripts/app_store_readiness_status.sh` shows whether CaptainsLog and Kit941 are synced with upstream. CaptainsLog is currently clean and synced with `origin/main`. Kit941 is currently clean but locally ahead of `origin/main` at `f06a0750122f` by `Playground/PlaygroundApp.swift` only; the linked library source remains at the pushed `cbac28644070` content. If compiled library source is ever unsynced, the export manifest must record the exact local commit and the release owner must explicitly accept that state. |
