...
| Apple Watch / Apple TV Availability | No action for the first release unless the platform work is intentionally finished | Captain's Log now has first-pass watchOS and tvOS companion targets that build and launch with signing disabled, but they are not App Store-ready. Return and Get Bananas remain useful local precedents, not substitute evidence. Do not enable Watch or TV availability until Captain's Log has real sync/setup, icons/top-shelf assets, App Store screenshots, signed export, TestFlight, and platform QA. |
