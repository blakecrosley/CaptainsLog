...
| Apple Watch / Apple TV Availability | No action for the first release unless the platform work is intentionally finished | Captain's Log now has watchOS and tvOS companion targets that build, launch with signing disabled, and share an aggregate snapshot through WatchConnectivity plus iCloud key-value sync. Return and Get Bananas remain useful local precedents, not substitute evidence. Do not enable Watch or TV availability until Captain's Log has platform assets, App Store screenshots, signed export, TestFlight, provisioning validation, and platform QA. |
