...
3. Make distribution signing/export possible:
   ...
   For Mac/TV readiness, run the same helper with `--target macos` or `--target tvos` after the shared app record path is accepted. The helper creates profiles only after `--apply --confirm-team M4WTLM6RAQ`; without `--apply`, it is read-only against Apple account state unless `--download-existing` is used to install already-active remote profiles into Xcode's local provisioning profile directory.
