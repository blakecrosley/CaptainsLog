...
- Apple Watch and Apple TV: no action in App Store Connect for this release unless the companion targets are intentionally finished. They now compile and launch in unsigned simulator smokes as Captain's Log targets and have an aggregate snapshot data path, but still need platform assets, App Store screenshots, signed export, TestFlight, provisioning validation, and platform QA before availability.
