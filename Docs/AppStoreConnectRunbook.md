...
If export reports that the App Store provisioning profile lacks iCloud or `com.apple.developer.ubiquity-kvstore-identifier`, enable iCloud key-value storage for `com.blakecrosley.captainslog` in Apple Developer/App Store Connect, regenerate or download the App Store profile, then rerun `Scripts/app_store_signing_status.sh` before another export attempt.
