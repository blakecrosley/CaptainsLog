...
`Scripts/check_app_store_account_access.py` queries App Store Connect user visibility plus one aggregate app-list page, and prints only aggregate role/all-app/provisioning/app counts. It intentionally omits user names, emails, app names, bundle IDs, resource IDs, and API-key material. Passing this check proves the selected API credential can read account user visibility and list existing apps; visible role aggregates are still not selected-key proof, and the check does not prove app-record creation, signing-asset creation, or cloud-managed distribution certificate access. Apple's `CLOUD_MANAGED_APP_DISTRIBUTION` role is the cloud-managed Apple Distribution permission, but `xcodebuild -exportArchive` remains the authority for whether the selected credential can actually use it.
