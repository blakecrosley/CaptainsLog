...
| Distribution signing | `Scripts/app_store_signing_status.sh`, `security find-identity -v -p codesigning`, `Scripts/export_app_store_ipa.sh`, and readiness script | Current keychain has Apple Development and Developer ID Application identities, but no Apple Distribution/iOS Distribution identity. The local provisioning profile directory exists with 0 installed profiles. Export now stops immediately with setup guidance unless `CAPTAINS_LOG_SKIP_DISTRIBUTION_SIGNING_PRECHECK=1` is set. | Blocked |
