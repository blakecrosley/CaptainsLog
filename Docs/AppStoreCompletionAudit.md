...
| Current-head simulator tests | `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-uitesting-gate-tests -only-testing:CaptainsLogTests`, `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-period-menu-uitests -only-testing:CaptainsLogUITests` | Current `main` passed 69 unit tests and, after the compact dashboard period-menu polish, 2 UI tests with 0 failures on an iOS 26.5 simulator. The UI tests cover first-run primary actions, fixture dashboard, Settings, Privacy & Data, and selected-day journal detail. | Complete locally |
| Release simulator string scan | `xcodebuild build -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -configuration Release -destination 'id=00119EA5-FDF1-4F0B-A47F-5ADB10AD6BA3' -derivedDataPath /tmp/captainslog-uitesting-gate-release`, `strings <Release app executable>` | Release simulator build succeeded. The built executable did not contain the debug/test fixture strings that the future IPA `local-check` rejects, including screenshot hooks, debug OpenAI key seeding hooks, UI-testing hooks, and the screenshot demo key. Earlier bundle metadata also showed the expected bundle ID, version/build, export-compliance flag, and privacy manifest. | Complete locally; signed IPA local-check still required |
