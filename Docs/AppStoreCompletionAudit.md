...
| Linked package source custody | `Scripts/app_store_readiness_status.sh`, `git -C ../941Kit status --short --branch` | Kit941 package source is clean, but the local Kit941 branch is ahead of `origin/main` by one commit. This does not dirty the export, but it should be pushed or intentionally accepted before final release for source traceability. | Warning |
