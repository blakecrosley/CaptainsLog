...
| App Store Connect API key custody | `Scripts/upload_app_store_ipa.sh credential-guard-self-test`, `Scripts/app_store_readiness_status.sh`, `.gitignore` | Credential guard self-test passed. Readiness confirms no `.p8` private-key material is in the repo. `.gitignore` ignores local env files, `private_keys/`, `.p8`, `.mobileprovision`, `.provisionprofile`, `.cer`, and `.p12` files so common App Store signing artifacts are not staged accidentally. | Complete locally, real credentials open |
