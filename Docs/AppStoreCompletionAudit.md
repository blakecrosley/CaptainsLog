...
| Current-head simulator tests | `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-doc-refresh-unit-tests -only-testing:CaptainsLogTests`, `xcodebuild test -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-doc-refresh-ui-tests -only-testing:CaptainsLogUITests` | Current `main` passed 69 unit tests and 2 UI tests with 0 failures on the iOS 26.5 iPhone 17 simulator. The UI tests cover first-run primary actions, fixture dashboard, Settings, Privacy & Data, and selected-day journal detail. | Complete locally |
| Release simulator string scan | `xcodebuild build -project CaptainsLog.xcodeproj -scheme CaptainsLog-iOS -configuration Release -destination 'id=277C8808-F02C-43A4-8B4A-11BA187F0788' -derivedDataPath /tmp/captainslog-doc-refresh-release`, `strings <Release app executable>` | Release simulator build succeeded on the iOS 26.5 iPhone 17 simulator. The built executable did not contain the debug/test fixture strings that the future IPA `local-check` rejects, including screenshot hooks, debug OpenAI key seeding hooks, UI-testing hooks, and the screenshot demo key. Bundle metadata also showed the expected bundle ID, version/build, export-compliance flag, and privacy manifest. | Complete locally; signed IPA local-check still required |
