...
| "does this app have a mac version ready?" | `CaptainsLog-macOS` exists with bundle ID `com.blakecrosley.captainslog.mac`, automatic signing team `M4WTLM6RAQ`, local smoke evidence, and Mac screenshots. REST checks report the native Mac Developer Portal bundle ID is missing. | Target exists, not Mac App Store ready. | Create the native Mac bundle ID/capabilities, regenerate profiles, produce a signed Mac App Store package, then TestFlight and QA. |
...
| "does this app have an Apple TV version ready?" | `CaptainsLog-tvOS` exists with bundle ID `com.blakecrosley.captainslog.tv`, local launch/screenshot/OCR evidence, top-shelf assets, and an iCloud read-only snapshot path. REST checks report the TV Developer Portal bundle ID is missing. | Companion exists locally, not App Store ready. | Create the TV bundle ID/capabilities, regenerate profiles, prove signed archive/export, TestFlight, living-room QA, and screenshot acceptance. |
...
| "we have all that in other apps, like Return, Get Bananas" | Return, ReturnTV, Return Watch, and Get Bananas Watch provide exact 941 bundle-ID precedents with `platform: UNIVERSAL` and required capabilities. | Useful precedent only. They do not create Captain's Log bundle IDs, app records, profiles, signed exports, or TestFlight evidence. | Use the precedent to run the guarded Captain's Log account step: `Scripts/ensure_platform_bundle_ids.py --apply --confirm-team M4WTLM6RAQ`, then verify with `Scripts/check_app_store_connect_record.py`. |
