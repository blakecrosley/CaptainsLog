...
| --- | --- | --- | --- |
| "does this app have an iPad version ready?" | `project.yml` sets the iOS target to `TARGETED_DEVICE_FAMILY: "1,2"`; readiness reports iPhone/iPad enabled. A current May 20, 2026 no-sign Release smoke built `/tmp/captainslog-ipad-smoke-release-build/Build/Products/Release-iphonesimulator/Captain's Log.app`, verified bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, `UIDeviceFamily: [1,2]`, installed it on an iPad simulator, launched process `13092`, and skipped screenshot capture. | Locally yes, store no. It ships through the universal iOS app after signed IPA export and App Store Connect upload. | Create the App Store Connect app record, make distribution signing work, reconcile linked Kit941 source, regenerate the signed IPA/export manifest, then upload/TestFlight. |
| "does this app have a mac version ready?" | `CaptainsLog-macOS` exists with shared bundle ID `com.blakecrosley.captainslog`, automatic signing team `M4WTLM6RAQ`, and local smoke evidence. A current May 20, 2026 no-sign Release smoke built `/tmp/captainslog-macos-release-build/Build/Products/Release/Captain's Log.app`, verified bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, App Store category `public.app-category.developer-tools`, launched process `7487`, and quit cleanly. Apple says added Mac platform versions in a single app record/universal purchase share the iOS app bundle ID; Get Bananas uses that main-bundle-ID pattern. Current remote signing evidence has Mac App Store application signing visible through Apple Distribution, but still lacks a usable Mac installer certificate and active `MAC_APP_STORE` profile. | Target exists, not Mac App Store ready. | Produce a signed Mac App Store package and `MacExportManifest.txt`, then complete TestFlight, store-media acceptance, and QA. |
| "does this app have an Apple Watch version ready?" | `CaptainsLog-watchOS` exists with bundle ID `com.blakecrosley.captainslog.watchkitapp`, unsigned local launch evidence, and a phone-synced aggregate snapshot path. A current May 20, 2026 no-sign Release smoke built `/tmp/captainslog-watchos-smoke-release-build/Build/Products/Release-watchsimulator/Captain's Log.app`, installed it on the Watch simulator, launched process `8001`, and skipped screenshot/OCR capture. REST checks report the Watch Developer Portal bundle ID is missing. | Companion exists locally, not App Store ready. | After explicit account-mutation approval, create the Watch bundle ID/capabilities, regenerate profiles, prove signed archive/export, TestFlight, paired-device QA, and store-media acceptance. |
| "does this app have an Apple TV version ready?" | `CaptainsLog-tvOS` exists with shared bundle ID `com.blakecrosley.captainslog`, unsigned local launch evidence, top-shelf assets, and an iCloud read-only snapshot path. A current May 20, 2026 no-sign Release smoke built `/tmp/captainslog-tvos-smoke-release-build/Build/Products/Release-appletvsimulator/Captain's Log.app`, installed it on the TV simulator, launched process `8235`, and skipped screenshot/OCR capture. Return's tvOS target uses the same bundle ID as its iOS app. Current remote signing evidence has the required iOS distribution certificate visible, but still lacks an active `TVOS_APP_STORE` profile. | Target exists locally, not App Store ready. | Regenerate the required TV App Store signing/profile path, prove signed archive/export, TestFlight, living-room QA, and store-media acceptance. |
| "put it in the vision store too" | The iOS target supports compatible Vision availability; the no-screenshot Vision smoke builds, installs, and launches the compatible app on a Vision simulator. The May 20, 2026 run recorded process `7756` with `screenshot=skipped`. | Use compatible iPhone/iPad availability, not native visionOS. Store readiness still depends on signed iOS upload and App Store Connect availability selection. | After signed iOS upload, select compatible Apple Vision Pro availability in App Store Connect and complete human acceptance. |
