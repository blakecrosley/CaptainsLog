...
- Mac: a native macOS target exists and now shares the iOS app bundle ID `com.blakecrosley.captainslog` for the single App Store record/universal-purchase path. Mac App Store availability remains blocked on signing/export, TestFlight, screenshot acceptance, and human QA.
...
- Apple TV: read-only companion target exists, now shares the iOS app bundle ID `com.blakecrosley.captainslog`, compiles/launches with signing disabled, reads the same aggregate snapshot through iCloud key-value sync, has app icon/top-shelf assets, and has a local App Store screenshot capture. It is not ready until signed build/export, TestFlight, App Store platform availability, TV QA, provisioning validation, and human screenshot acceptance are complete.

Current May 19, 2026 account/signing audit:

- `Scripts/app_store_readiness_status.sh` still fails local readiness because the current iOS IPA and export manifest are missing.
- `Scripts/check_remote_signing_assets.py --require` reports 0 usable visible iOS distribution certificates, 0 usable visible Mac App Store application certificates, 0 usable visible Mac installer certificates, an invalid visible `IOS_APP_STORE` profile for `com.blakecrosley.captainslog`, no required active iOS/Mac/tvOS App Store profiles, and the Watch companion bundle ID missing.
...
| "does this app have a mac version ready?" | `CaptainsLog-macOS` exists with shared bundle ID `com.blakecrosley.captainslog`, automatic signing team `M4WTLM6RAQ`, local smoke evidence, and Mac screenshots. A current May 19, 2026 no-sign Release build succeeded at `/tmp/captainslog-current-macos-nosign-build` with bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, and privacy manifest present. Apple says added Mac platform versions in a single app record/universal purchase share the iOS app bundle ID; Get Bananas uses that main-bundle-ID pattern. Current remote signing evidence still has no usable Mac App Store application or installer certificate/profile path. | Target exists, not Mac App Store ready. | Produce a signed Mac App Store package and `MacExportManifest.txt`, then complete TestFlight, screenshot acceptance, and QA. |
| "does this app have an Apple Watch version ready?" | `CaptainsLog-watchOS` exists with bundle ID `com.blakecrosley.captainslog.watchkitapp`, local launch/screenshot/OCR evidence, and a phone-synced aggregate snapshot path. A current May 19, 2026 no-sign Release build succeeded at `/tmp/captainslog-current-watchos-nosign-build` with version `1.0.0 (1)` and privacy manifest present. REST checks report the Watch Developer Portal bundle ID is missing. | Companion exists locally, not App Store ready. | After explicit account-mutation approval, create the Watch bundle ID/capabilities, regenerate profiles, prove signed archive/export, TestFlight, paired-device QA, and screenshot acceptance. |
| "does this app have an Apple TV version ready?" | `CaptainsLog-tvOS` exists with shared bundle ID `com.blakecrosley.captainslog`, local launch/screenshot/OCR evidence, top-shelf assets, and an iCloud read-only snapshot path. A current May 19, 2026 no-sign Release build succeeded at `/tmp/captainslog-current-tvos-nosign-build` with version `1.0.0 (1)` and privacy manifest present. Return's tvOS target uses the same bundle ID as its iOS app. Current remote signing evidence still has no usable iOS distribution certificate path or active `TVOS_APP_STORE` profile. | Target exists locally, not App Store ready. | Regenerate the required TV App Store signing/profile path, prove signed archive/export, TestFlight, living-room QA, and screenshot acceptance. |
