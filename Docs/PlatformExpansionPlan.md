...
- `Scripts/check_remote_signing_assets.py --require` reports 1 usable / 1 visible iOS App Store distribution certificate, 1 usable / 1 visible Mac App Store application certificate through Apple Distribution, 0 usable visible Mac installer certificates, an invalid visible `IOS_APP_STORE` profile for `com.blakecrosley.captainslog`, no required active iOS/Mac/tvOS App Store profiles, and the Watch companion bundle ID missing.
...
| "does this app have a mac version ready?" | `CaptainsLog-macOS` exists with shared bundle ID `com.blakecrosley.captainslog`, automatic signing team `M4WTLM6RAQ`, local smoke evidence, and Mac screenshots. A current May 19, 2026 no-sign Release build succeeded at `/tmp/captainslog-current-macos-nosign-build` with bundle ID `com.blakecrosley.captainslog`, version `1.0.0 (1)`, and privacy manifest present. Apple says added Mac platform versions in a single app record/universal purchase share the iOS app bundle ID; Get Bananas uses that main-bundle-ID pattern. Current remote signing evidence has Mac App Store application signing visible through Apple Distribution, but still lacks a usable Mac installer certificate and active `MAC_APP_STORE` profile. | Target exists, not Mac App Store ready. | Produce a signed Mac App Store package and `MacExportManifest.txt`, then complete TestFlight, screenshot acceptance, and QA. |
